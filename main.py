...
if not WIDGET_URL:
    print("⚠️  WIDGET_URL not set, widget will not load")

# HTML template with external widget references (OpenAI Apps SDK standard)
# Widget JS and CSS served from Render static site
WIDGET_HTML = f'''
<div id="root"></div>
...
async def _list_resources() -> List[types.Resource]:
    ...
    return [
        types.Resource(
            uri=WIDGET_URI,
            name="Hello World Widget UI",
            title="Hello World Widget UI",
...
async def _list_tools() -> List[types.Tool]:
    ...
    return [
        types.Tool(
            name="say_hello",
            title="Say Hello",
            description="Generate a personalized hello world greeting with optional name and message",
            inputSchema={
                "type": "object",
                "properties": {
                    "name": {
                    ...
                    "message": {
                        "type": "string",
                        "description": "Custom message to include in the greeting (optional)",
                    },
                },
                "required": [],
...
async def _call_tool_request(
    ...
    if tool_name == "say_hello":
        ...
        full_message = f"{greeting} {message}" if message else greeting
        ...
        return types.CallToolResult(
            content=[
                types.TextContent(
                    type="text",
                    text=f"{full_message} Interact with the greeting component below.",
                )
            ],
            structuredContent={
                "greeting": greeting,
                "name": name,
                "summary": f"Generated personalized greeting for {name}",
            },
            _meta={
                "fullData": data,
            },
